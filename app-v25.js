  ...
  tryFeed("Activities",d.activitiesCsv,acts=>{
   state.activities=acts
    ...
    .map(x=>({
     ...
     image:x.Image||fallback.activities[0].image
    }));
  },renderActivities),

  tryFeed("Signups",d.signupsCsv,sups=>{
   state.signups=sups.map(x=>({
    activityId:x.ActivityID||x["Activity ID"],
    status:x.Status||"Going",
    people:Number(x.People||1)
   }));
   signupsReady=true;
  },renderActivities),
 ...
 $("#activityGrid").innerHTML=a.map(x=>`
  <article class="activity-card">
   ...
   <div class="activity-body">
    ...
    <div class="activity-meta">
     ${signupsReady
       ? `<strong>👥 ${countFor(x.id,"Going")} going</strong><span>${countFor(x.id,"Maybe")} maybe</span>`
       : `<strong>👥 Loading head count…</strong><span></span>`
     }
    </div>
...
console.info("[Trip Site] V2.5 started");
renderAll();
countdown();
loadData();
