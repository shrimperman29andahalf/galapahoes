...
async function loadData(){
 try{
  const d=CFG.data||{},[a,acts,sups,c,f]=await Promise.all([loadCsv(d.announcementsCsv),loadCsv(d.activitiesCsv),loadCsv(d.signupsCsv),loadCsv(d.crewCsv),loadCsv(d.flightsCsv)]);
  if(a?.length)state.announcements=a.map(x=>({date:x.Date||x.date,tag:x.Tag||x.tag||"UPDATE",title:x.Title||x.title,body:x.Body||x.body}));
  if(acts?.length)state.activities=acts.filter(x=>(x.Approved||x.approved||"YES").toUpperCase()!=="NO").map(x=>({id:x.ID||x.id,name:x.Activity||x.activity,date:[x.Date||x.date,x.Time||x.time].filter(Boolean).join(" · "),proposer:x.ProposedBy||x["Proposed By"]||"",description:x.Description||"",cost:x.Cost||"TBD",min:Number(x.Minimum||0),status:x.Status||"Open",image:x.Image||fallback.activities[0].image}));
  if(sups?.length)state.signups=sups.map(x=>({activityId:x.ActivityID||x["Activity ID"],status:x.Status||"Going",people:Number(x.People||1)}));
  if(c?.length)state.crew=c.map(x=>({name:x.Name||x.name,note:x.Note||x.note||"Going"}));
  if(f?.length)state.flights=f.map(x=>({name:x.Name||x.name,arrival:x.Arrival||x.arrival,arrivalFlight:x.ArrivalFlight||x["Arrival Flight"]||"",departure:x.Departure||x.departure,departureFlight:x.DepartureFlight||x["Departure Flight"]||""}));
 }catch(e){console.warn("Using sample data",e)}
 renderAll()
}
...
function renderActivities(filter="all"){let a=state.activities;if(filter==="open")a=a.filter(x=>String(x.status).toLowerCase()==="open");if(filter==="free")a=a.filter(x=>/free|\$0/i.test(x.cost));if(filter==="paid")a=a.filter(x=>!/free|\$0/i.test(x.cost));$("#activityCount").textContent=a.length;$("#activityGrid").innerHTML=a.map(x=>`<article class="activity-card"><div class="activity-img" style="background-image:url('${x.image}')"></div><div class="activity-body"><div class="activity-top"><div><div class="activity-date">${x.date}</div><h3>${x.name}</h3></div><div class="activity-cost">${x.cost}</div></div><p class="activity-desc">${x.description}</p><div class="activity-meta"><span>Proposed by ${x.proposer||"the group"}</span><span>${x.min?`Min. ${x.min}`:"Open group"}</span></div><div class="activity-meta"><strong>👥 ${countFor(x.id,"Going")} going</strong><span>${countFor(x.id,"Maybe")} maybe</span></div><div class="activity-actions"><button class="going" onclick="openSignup('${x.id}',${JSON.stringify(x.name)},'Going')">I'm in</button><button onclick="openSignup('${x.id}',${JSON.stringify(x.name)},'Maybe')">Maybe</button></div></div></article>`).join("")}
